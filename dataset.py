...
class CodeSearchDataset(Dataset):
  ...
  # 从jsonl中读取所需要的数据，其中每一项数据都是InputFeatures类型，其中包含了每条数据的idx、url、nl信息和pl信息
    def load_example(self,mode):
        examples = []
        if mode == 'test':
            test_path = config.data_path + "/java_test_0.jsonl"
            ...
            eval_path = config.data_path + "/java_valid_0.jsonl"
            ...
            num = 0
            train_path = config.data_path +"/java_train_"
            for i in range(1):
                postfix = str(i)+".jsonl"
...
class ClassifierDataset(Dataset):
    ...
    def load_examples(self,mode):
        if mode == 'eval':
            ...
            examples = []
            train_path = config.data_path+"/classifier/ctrain_0.jsonl"
            with open(train_path,'r') as f:
                for line in f.readlines():
                    js = json.loads(line)
                    example = convert_examples_to_features(js,config.tokenizer,-1,True)
                    examples.append(example)
            return examples
